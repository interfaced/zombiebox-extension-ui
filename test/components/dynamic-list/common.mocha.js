...
describe('DynamicList common', () => {
	...
	describe('Instance', () => {
		...
		describe('Public methods', () => {
			...
			it('preload should returns Promise with DynamicList', () => instance
				.preload()
				.then((result) => {
					expect(result).instanceOf(DynamicList);
				}));

			it('loadItems should returns Promise with DynamicList', () => instance
				.loadItems(0, 10)
				.then((result) => {
					expect(result).instanceOf(DynamicList);
				}));

			it('loadNextFrame should returns Promise with DynamicList', () => {
				expect(instance.loadNextFrame()).instanceOf(Promise);
			});

			it('loadPrevFrame should returns Promise with DynamicList', () => {
