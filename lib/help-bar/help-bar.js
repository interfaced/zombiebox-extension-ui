/*
 * This file is part of the ZombieBox package.
 *
 * Copyright (c) 2011-2015, Interfaced
 *
 * For the full copyright and license information, please view the LICENSE
 * file that was distributed with this source code.
 */
goog.provide('zb.ui.HelpBar');
goog.require('zb.html');
goog.require('zb.ui.HelpBarItem');
goog.require('zb.ui.IHelpBarItem');
goog.require('zb.ui.templates.helpBar');
goog.require('zb.widgets.CuteWidget');



/**
 * @param {zb.ui.HelpBar.Input=} opt_params
 * @extends {zb.widgets.CuteWidget}
 * @constructor
 */
zb.ui.HelpBar = function(opt_params) {
	goog.base(this);

	var itemClass = zb.ui.HelpBarItem;
	if (opt_params && opt_params.itemClass) {
		itemClass = opt_params.itemClass;
	}
	this._itemClass = itemClass;

	this._items = [];
	this._itemsOrder = null;
};
goog.inherits(zb.ui.HelpBar, zb.widgets.CuteWidget);


/**
 * @param {zb.device.input.Keys} zbKey
 * @param {KeyboardEvent=} opt_event
 * @return {boolean}
 */
zb.ui.HelpBar.prototype.processHelpBarKey = function(zbKey, opt_event) {
	var isHandled = false;

	if (this.getEnabled() && this.getVisible()) {
		isHandled = this._items.some(function(item) {
			return item.processHelpBarKey(zbKey, opt_event);
		});
	}

	return isHandled;
};


/**
 * @param {zb.ui.HelpBarItem.Options} options
 * @return {zb.ui.IHelpBarItem}
 */
zb.ui.HelpBar.prototype.createItem = function(options) {
	return new this._itemClass(options);
};


/**
 * @param {Array.<?zb.ui.IHelpBarItem>} items
 */
zb.ui.HelpBar.prototype.setItems = function(items) {
	this.clear();

	// exclude null items
	items = items
		.filter(function(item) {
			return item !== null;
		});

	// sort items by zbKeys
	if (this._itemsOrder) {
		this._sortItems(this._itemsOrder, items);
	}

	// render items
	items.forEach(this._addItem, this);
};


/**
 * @param {?Array.<zb.device.input.Keys>} order
 */
zb.ui.HelpBar.prototype.setOrder = function(order) {
	this._itemsOrder = order;
};


/**
 *
 */
zb.ui.HelpBar.prototype.clear = function() {
	while (this._items.length) {
		this._removeItem(0);
	}
};


/**
 * @param {zb.device.input.Keys} zbKey
 * @return {zb.ui.IHelpBarItem}
 */
zb.ui.HelpBar.prototype.getItem = function(zbKey) {
	for (var i = this._items.length; i--;) {
		if (this._items[i].isMyKey(zbKey)) {
			return this._items[i];
		}
	}

	return null;
};


/**
 * @inheritDoc
 */
zb.ui.HelpBar.prototype.isFocusable = function() {
	var hasItems = this._items.length > 0;
	if (hasItems) {
		return goog.base(this, 'isFocusable');
	} else {
		return false;
	}
};


/**
 * @inheritDoc
 */
zb.ui.HelpBar.prototype._renderTemplate = function() {
	return zb.ui.templates.helpBar(this._getTemplateData(), this._getTemplateOptions());
};


/**
 * @param {zb.ui.IHelpBarItem} item
 * @protected
 */
zb.ui.HelpBar.prototype._addItem = function(item) {
	this.appendWidget(item);
	this._exported.items.appendChild(item.getContainer());
	this._items.push(item);
};


/**
 * @param {number} index
 * @protected
 */
zb.ui.HelpBar.prototype._removeItem = function(index) {
	var item = this._items[index];
	this.removeWidget(item);
	zb.html.remove(item.getContainer());
	this._items.splice(index, 1);
};


/**
 * Sort items by zbKey in order [...anyCustomKeysInAddOrder, ...commonKeysByDefinedOrder]
 * @param {Array.<zb.device.input.Keys>} order
 * @param {Array.<zb.ui.IHelpBarItem>} items
 */
zb.ui.HelpBar.prototype._sortItems = function(order, items) {
	/**
	 * @param {?zb.ui.IHelpBarItem} item
	 * @return {number}
	 */
	var getItemWeight = function(item) {
		var keys = item.getKeys();
		return Math.max.apply(null, keys.map(function(zbKey) {
			return order.indexOf(zbKey);
		}));
	};

	items.sort(function(itemA, itemB) {
		return getItemWeight(itemA) - getItemWeight(itemB);
	});
};


/**
 * @type {zb.ui.templates.HelpBarOut}
 * @protected
 */
zb.ui.HelpBar.prototype._exported;


/**
 * @type {Function}
 * @protected
 */
zb.ui.HelpBar.prototype._itemClass;


/**
 * @type {Array.<zb.ui.IHelpBarItem>}
 * @protected
 */
zb.ui.HelpBar.prototype._items;


/**
 * @type {?Array.<zb.device.input.Keys>}
 * @protected
 */
zb.ui.HelpBar.prototype._itemsOrder;


/**
 * @typedef {{
 *     itemClass: (Function|undefined)
 * }}
 */
zb.ui.HelpBar.Input;
