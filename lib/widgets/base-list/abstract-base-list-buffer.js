goog.provide('zb.ui.widgets.AbstractBaseListBuffer');
goog.require('zb.ui.data.IList');
goog.require('zb.ui.widgets.BaseListUtils');


/**
 * @abstract
 */
zb.ui.widgets.AbstractBaseListBuffer = class {
	/**
	 * @param {zb.ui.widgets.AbstractBaseListBuffer.Options} options
	...
	getSourceEnd() {
		...
		return zb.ui.widgets.BaseListUtils.sizeToEnd(sourceSize);
	}
	...
	getLocalStart() {
		...
		const globalCenterLineStart = zb.ui.widgets.BaseListUtils.getLineStart(this._globalCenter, lineSize);
		...
		if (localStart < sourceStart) {
			if (this.isGlobalStart()) {
				...
				localStart = zb.ui.widgets.BaseListUtils.getLineStart(sourceStart, lineSize);
	...
	getLocalEnd() {
		const lineSize = this._options.lineSize;
		const globalCenterLineEnd = zb.ui.widgets.BaseListUtils.getLineEnd(this._globalCenter, lineSize);
		const paddingItems = this.getItemsByLines(this._options.padding);
		...
		if (localEnd > sourceEnd) {
			if (this.isGlobalEnd()) {
				...
				localEnd = zb.ui.widgets.BaseListUtils.getLineEnd(sourceEnd, lineSize);
	...
	getLocalSize() {
		...
		return zb.ui.widgets.BaseListUtils.endToSize(localEnd);
	}
	...
	getGlobalSize() {
		...
		return zb.ui.widgets.BaseListUtils.endToSize(this.getGlobalEnd());
	}
	...
	getItemsByLines(line) {
		return zb.ui.widgets.BaseListUtils.getItemsByLines(line, this._options.lineSize);
	}
	...
	getLinesByItems(items) {
		return zb.ui.widgets.BaseListUtils.getLinesByItems(items, this._options.lineSize);
	}
	...
	getLineByIndex(index) {
		return zb.ui.widgets.BaseListUtils.getLineByIndex(index, this._options.lineSize);
	}
	...
	selectNextLine() {
		...
		let nextGlobalIndex = zb.ui.widgets.BaseListUtils.getNextLine(currGlobalIndex, this._options.lineSize);
		...
		if (this.isCyclical() && nextGlobalIndex > lastGlobalIndex) {
			...
			const firstGlobalLineEnd = zb.ui.widgets.BaseListUtils.getLineEnd(firstGlobalIndex, this._options.lineSize);
			const nextGlobalLineEnd = zb.ui.widgets.BaseListUtils.getLineEnd(nextGlobalIndex, this._options.lineSize);
			...
			const possibleNextGlobalIndex = zb.ui.widgets.BaseListUtils.getNextIndex(currGlobalIndex);
	...
	selectPrevLine() {
		const currGlobalIndex = this.getGlobalIndex();
		let prevGlobalIndex = zb.ui.widgets.BaseListUtils.getPrevLine(currGlobalIndex, this._options.lineSize);

		if (this.isCyclical() && prevGlobalIndex < 0) {
			const lastGlobalIndex = this.getGlobalEnd();
			const lastGlobalLineEnd = zb.ui.widgets.BaseListUtils.getLineEnd(lastGlobalIndex, this._options.lineSize);
			const prevGlobalLineEnd = zb.ui.widgets.BaseListUtils.getLineEnd(prevGlobalIndex, this._options.lineSize);

			prevGlobalIndex = lastGlobalLineEnd - (prevGlobalLineEnd - prevGlobalIndex);

			if (prevGlobalIndex > lastGlobalIndex) {
				prevGlobalIndex = zb.ui.widgets.BaseListUtils.getPrevLine(prevGlobalIndex, this._options.lineSize);
			}
		}
	...
	selectNextIndex() {
		...
		if (this.isCyclical() && currGlobalIndex === lastGlobalIndex) {
			...
			nextGlobalIndex = zb.ui.widgets.BaseListUtils.getNextIndex(currGlobalIndex);
		}
	...
	selectPrevIndex() {
		...
		if (this.isCyclical() && currGlobalIndex === 0) {
			...
			prevGlobalIndex = zb.ui.widgets.BaseListUtils.getPrevIndex(currGlobalIndex);
		}
	 ...
	 */
	_updateGlobalCenter() {
		// Переменная globalIndex может быть NaN, когда установлен пустой zb.ui.data.List.
	...
	_isChangeNeeded(sourceIndex) {
		...
		const globalIndexLineStart = zb.ui.widgets.BaseListUtils.getLineStart(globalIndex, lineSize);
		let loadOnLeftStart = globalIndexLineStart - loadOnLeftItems;
		loadOnLeftStart = Math.max(globalStart, loadOnLeftStart);

		const globalIndexLineEnd = zb.ui.widgets.BaseListUtils.getLineEnd(globalIndex, lineSize);
 ...
 * @protected
 */
zb.ui.widgets.AbstractBaseListBuffer.prototype._changeCallback;
 ...
 * @protected
 */
zb.ui.widgets.AbstractBaseListBuffer.prototype._selectCallback;


/**
 * @type {zb.ui.widgets.AbstractBaseListBuffer.Source}
 * @protected
 */
zb.ui.widgets.AbstractBaseListBuffer.prototype._source;
 ...
 * @private
 */
zb.ui.widgets.AbstractBaseListBuffer.prototype._options;
 ...
 * @private
 */
zb.ui.widgets.AbstractBaseListBuffer.prototype._globalCenter;
 ...
 * @private
 */
zb.ui.widgets.AbstractBaseListBuffer.prototype._globalEnd;
 ...
 * @private
 */
zb.ui.widgets.AbstractBaseListBuffer.prototype._isBlocked;
 ...
 * @private
 */
zb.ui.widgets.AbstractBaseListBuffer.prototype._localItems;
 ...
 * @private
 */
zb.ui.widgets.AbstractBaseListBuffer.prototype._currentItem;
 ...
 * }}
 */
zb.ui.widgets.AbstractBaseListBuffer.Options;


/**
 * @typedef {?zb.ui.data.IList}
 */
zb.ui.widgets.AbstractBaseListBuffer.Source;
