...
zb.ui.BaseListBuffer = class {
	...
	_isChangeNeeded(sourceIndex) {
		...
		const isGlobalStart = globalIndex - loadOnLeftItems < this.getGlobalStart();
		const isGlobalEnd = globalIndex + loadOnLeftItems > this.getGlobalEnd();

		const isLocalStart = globalIndex - loadOnLeftItems < this.getLocalStart();
		const isLocalEnd = globalIndex + loadOnLeftItems > this.getLocalEnd();

		const isOnlyLocalStart = isLocalStart && !isGlobalStart;
		const isOnlyLocalEnd = isLocalEnd && !isGlobalEnd;
	...
	_doSelectItem(newItem, newSourceIndex, oldItem, oldSourceIndex) {
		...
		const isItemsEqual = newItem === oldItem;
