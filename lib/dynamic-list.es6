...
zb.ui.DynamicList = class extends zb.ui.DataList {
	...
	loadItems(from, to, opt_prepend) {
		this._query = this
			...
			.then(items => {
				...
				if (bufferOverflow > 0) {
					let newBufferStart = bufferStart + (!opt_prepend ? 1 : -1) * bufferOverflow;
					let removeFrom = !opt_prepend ? 0 : bufferSize;
					let removeTo = removeFrom + bufferOverflow;
	...
	loadNextFrame() {
		...
		var query = this.loadItems(from, to, false);
	...
	loadPrevFrame() {
		...
		var query = this.loadItems(from, to, true);
		this._fireEvent(this.EVENT_LOADING_DATA, query);
	...
	_configure(opt_options) {
		...
		for (const p in this._dlDefaultOptions) if (this._dlDefaultOptions.hasOwnProperty(p)) {
